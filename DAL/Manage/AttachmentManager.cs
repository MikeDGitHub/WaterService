...
namespace DAL.Manage
{
    public class AttachmentManager
    {
        ...
        {
            var sql = string.Format("insert into waterService.attachmentinfo (MeterId,ImgUrl,`Create`,CreateDate,GenreId,Resourceid) values({0},'{1}','{2}','{3}',{4},{5});", attachment.MeterId, attachment.ImgUrl, attachment.Create, attachment.Create, DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss"), attachment.Resourceid);
        ...
        public bool AddList(List<AttachmentInfo> list, object id, string Create, DateTime CreateDate, int GenreId)
        {
            ...
            list?.ForEach(item =>
            {
                sb.AppendFormat("insert into waterService.attachmentInfo (MeterId,ImgUrl,`Create`,CreateDate,GenreId,Resourceid) values({0},'{1}','{2}','{3}',{4},{5});", id, item.ImgUrl, Create, DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss"), GenreId, item.Resourceid);
            });
        ...
        public bool AddOrUpdate(List<AttachmentInfo> list)
        {
            ...
            list?.ForEach(item =>
        {
            sb.AppendFormat("insert into waterService.attachmentInfo (MeterId,ImgUrl,`Create`,CreateDate,GenreId,Resourceid) values({0},'{1}','{2}','{3}',{4},{5});", item.MeterId, item.ImgUrl, item.Create, DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss"), item.GenreId, item.Resourceid);
        });
            if (sb.Length > 1)
            {
                return new MySqlHelper().ExcuteNonQuery(sb.ToString()) > 0;
