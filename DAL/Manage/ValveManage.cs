...
namespace DAL.Manage
{
    public class ValveManage
    {
        public bool Add(UserInfo user, ValveInfo valve, List<AttachmentInfo> list)
        {
            ...
            sb.AppendFormat("insert into WaterService.ValveInfo(ValveCode,ValveName,TypeId,GenreId,Caliber,Lat,Lon,`Create`,CreateDate) values('{0}','{1}',{2},{3},{4},{5},{6},'{7}','{8}');select @@IDENTITY;", valve.ValveCode, valve.ValveName, valve.TypeId, valve.GenreId, valve.Caliber, valve.Lat, valve.Lon, user.Create, user.CreateDate);
        ...
        public bool Update(UserInfo user, ValveInfo valve, List<AttachmentInfo> list)
        {
            ...
            if (valve != null)
            {
                sb.AppendFormat("update WaterService.ValveInfo set GenreId={0},TypeId={1},Caliber={2},Lat={3},Lon={4},Modify='{5}',ModifyDate='{6}',ValveCode='{7}',ValveName='{8}' where ValveId={9};", valve.GenreId, valve.TypeId, valve.Caliber, valve.Lat, valve.Lon, valve.Modify, valve.ModifyDate, valve.ValveCode, valve.ValveName, valve.ValveId);
            }
